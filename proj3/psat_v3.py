...
def main(constant_fk2d,multiplying_factor_3d,Shear_velocity,tch,corr,SNR,Lambda,k,sch):
...
#page configuration
st.set_page_config(page_title='PSAT V3',page_icon=':cyclone:',layout='wide')

#sidebar included
with st.sidebar:
    select=option_menu(menu_title='Main Menu',options=['Home','Main Program','Made By'],styles={
        "container": {"padding": "5!important", "background-color": "#00172B"},
        "icon": {"color": "white", "font-size": "25px"}, 
        "nav-link": {"font-size": "16px", "text-align": "left", "margin":"0px", "--hover-color": "#eee"},
        "nav-link-selected": {"background-color": "#0083B8"}})
    
#if home is selected on sidebar
if select=='Home':
    st.header('Hello!') 
    st.subheader('This Is Project 3 For CS384') 
    st.write('##') 
    st.write('##')
    st.write('Welcome! Thank You For Visiting.') 
    st.write('Please note the following:')
    st.write('Enter All Values Correctly Before Computing')
    st.write('##')
    st.write('HAVE A WONDERFUL DAY!')
    
#if main is selected on sidebar    
if select=='Main Program':
    with st.container():
        st.header('This Is The Main Computation Program')
        st.subheader('Please Enter The Values Below')
        st.write('##')
        st.write('##')
        a=st.number_input('Enter Constant fk2d :')
        b=st.number_input('Enter Multiplying Factor 3D :')
        c=st.number_input('Enter Shear Velocity :')
        st.write('1. C')
        st.write('2. S')
        st.write('3. A')
        st.write('4. C & S')
        ...
        tch=st.number_input('What Is Your Choice Of Filteration ?')
        st.write('##')
        st.write('##')
        st.write('Based On Your Choice Above , Enter The Respective Values :')
        st.write('Leave Blank For Inputs Not Required !')
        C=st.number_input('Enter C :')
        S=st.number_input('Enter S :')
        AL=st.number_input('Enter Lambda Value For A :')
        Ak=st.number_input('Enter k Value For A :')
        st.write('##')
        st.write('##')
        st.write('Choose The Replacement Method From Below :')
        ...
        sch=st.number_input('What Is Your Choice ?')
        if st.button('Compute'):
            main(a,b,c,tch,C,S,AL,Ak,sch) #main program running in background console
            st.write('Computed')
            
#if made by is selected on sidebar         
if select=='Made By':
    with st.container():
